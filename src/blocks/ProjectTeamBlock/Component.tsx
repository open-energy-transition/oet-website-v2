...
export const ProjectTeamBlockComponent: React.FC<{
  title?: string | null
    ...
    <section className="container mx-auto px-4 py-12">
      ...
      {title && (
        <div className="mb-8 text-center">
          <h2 className="text-3xl w-full font-bold text-gray-900 dark:text-white mb-2">{title}</h2>
        </div>
      )}
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {teamMembers.map((item, index) => {
          ...
          const image = typeof member.image === 'object' ? member.image : null
          const fullName = `${member.firstName || ''} ${member.lastName || ''}`.trim()
          const { linkedIn, email, github, x, externalLink } = member
          ...
          return (
            ...
            >
              ...
              {/* Name */}
              <h3 className="text-xl font-semibold font-heebo text-center text-gray-900 dark:text-white w-full">
                {fullName}
              </h3>

              {/* Job Title */}
              {member.jobTitle && (
                <p className="mt-1 text-lg font-heebo font-normal text-gray-600 dark:text-gray-400 text-center w-full">
                  {member.jobTitle}
                </p>
              )}
              ...
              {/* Custom Description */}
              {item.description && (
                <p className="mt-2 text-gray-600 text-center dark:text-gray-300 text-sm w-full">
                  {item.description}
                </p>
              )}

              {/* Social Links */}
              <div className="flex gap-3.5 w-full justify-center mt-auto">
                {linkedIn && (
                  <a
                    href={linkedIn}
                    target="_blank"
