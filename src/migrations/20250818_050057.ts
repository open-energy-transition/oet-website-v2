...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  // Check and create types safely
  await db.execute(sql`
    DO $$
    BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_pages_blocks_our_service_link_type') THEN
        CREATE TYPE "public"."enum_pages_blocks_our_service_link_type" AS ENUM('reference', 'custom');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_pages_blocks_our_service_link_appearance') THEN
        CREATE TYPE "public"."enum_pages_blocks_our_service_link_appearance" AS ENUM('default', 'outline');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_pages_blocks_tools_we_support_link_type') THEN
        CREATE TYPE "public"."enum_pages_blocks_tools_we_support_link_type" AS ENUM('reference', 'custom');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_pages_blocks_tools_we_support_link_appearance') THEN
        CREATE TYPE "public"."enum_pages_blocks_tools_we_support_link_appearance" AS ENUM('default', 'outline');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_pages_blocks_who_we_are_link_type') THEN
        CREATE TYPE "public"."enum_pages_blocks_who_we_are_link_type" AS ENUM('reference', 'custom');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_pages_blocks_who_we_are_link_appearance') THEN
        CREATE TYPE "public"."enum_pages_blocks_who_we_are_link_appearance" AS ENUM('default', 'outline');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum__pages_v_blocks_our_service_link_type') THEN
        CREATE TYPE "public"."enum__pages_v_blocks_our_service_link_type" AS ENUM('reference', 'custom');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum__pages_v_blocks_our_service_link_appearance') THEN
        CREATE TYPE "public"."enum__pages_v_blocks_our_service_link_appearance" AS ENUM('default', 'outline');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum__pages_v_blocks_tools_we_support_link_type') THEN
        CREATE TYPE "public"."enum__pages_v_blocks_tools_we_support_link_type" AS ENUM('reference', 'custom');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum__pages_v_blocks_tools_we_support_link_appearance') THEN
        CREATE TYPE "public"."enum__pages_v_blocks_tools_we_support_link_appearance" AS ENUM('default', 'outline');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum__pages_v_blocks_who_we_are_link_type') THEN
        CREATE TYPE "public"."enum__pages_v_blocks_who_we_are_link_type" AS ENUM('reference', 'custom');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum__pages_v_blocks_who_we_are_link_appearance') THEN
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
    DO $$
    BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'b_bg') THEN
        CREATE TYPE "public"."b_bg" AS ENUM('#ffffff', '#000000', '#E41E3C', '#D7E4BF80');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'text_color') THEN
        CREATE TYPE "public"."text_color" AS ENUM('#000000', '#ffffff', '#D7E4BF80', '#E41E3C');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_models_links_link_type') THEN
        CREATE TYPE "public"."enum_models_links_link_type" AS ENUM('reference', 'custom');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_models_links_link_appearance') THEN
        CREATE TYPE "public"."enum_models_links_link_appearance" AS ENUM('default', 'github', 'internal', 'outline');
      END IF;

      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_footer_follow_us_link_actions_link_type') THEN
