...
import { generateSlug } from '@/utilities/extractHeadings'
...
const createJsxConverters =
  (enableHeadingIds: boolean = false): JSXConvertersFunction<NodeTypes> =>
  ({ defaultConverters }) => {
    ...
    const customDefaultConverters = {
      ...
      heading: ({ node, nodesToJSX }: any) => {
        ...
        // Generate ID from heading text if enableHeadingIds is true
        if (enableHeadingIds) {
...
export default function RichText(props: Props) {
  ...
  return (
    <ConvertRichText
      converters={createJsxConverters(enableHeadingIds)}
      className={cn(
        'payload-richtext',
        {
